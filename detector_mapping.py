...
#   build_detector_lane_mapping()  -> same as load_detectors_from_sumocfg()
# ======================================================================
...
def _parse_detectors_from_add_xml(add_xml_path: str) -> Dict[str, str]:
    ...
    Supported:
      <laneAreaDetector id="..." lane="..."/>
      <inductionLoop id="..." lane="..."/>
    """
    add_xml_path = _expand(add_xml_path)
    if not os.path.isfile(add_xml_path):
        return {}

    # ElementTree can't read .gz without gzip handling; skip gz files
...
def load_detectors_from_sumocfg(sumocfg_path: str) -> Dict[str, str]:
    """
    Main entrypoint:
      returns dict detector_id -> lane_id
    """
    ...
    if not det_to_lane:
        print(
            "[WARN] No detectors found from additional-files in .sumocfg. "
...
# Backward-compatible name expected by some code
